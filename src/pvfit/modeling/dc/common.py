...
class MaterialsInfo:
    ...
    def __post_init__(self) -> None:
        ...
        if not numpy.isfinite(self.E_g_eV_stc):
            raise ValueError(f"bandgap at STC is not well defined: {self.E_g_eV_stc}")

        if self.E_g_eV_stc <= 0:
            raise ValueError(
                f"bandgap at STC is less than or equal to zero: {self.E_g_eV_stc}"
            )
...
def get_scaled_thermal_voltage(
    ...
    parallel string. This expects inputs to already be validated.
    """
    return numpy.array(
        N_s
        * (
