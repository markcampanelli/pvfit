...
def fit(
    *, spec_sheet_parameters: SpecSheetParameters, method: str = "trf"
) -> types.ModelParametersFitResultLeastSquares:
    """
    ...
    dictionary with the following
        ...
        model_parameters
            Model parameters from fit starting at model_parameters_fittable_ic
        solver_result
    ...
    solver_result = scipy.optimize.least_squares(
        fun,
        x_0,
        **method_kwargs,
        x_scale="jac",
    ...
    if not solver_result.success:
        raise RuntimeError(
            f"least_squares solver returned an error, {solver_result.status}: "
            f"{solver_result.message}"
        )

    # Transform back fit values. Applies validation.
    model_parameters_fittable = types.ModelParametersFittable(
        I_rs_A_0=float(numpy.exp(solver_result.x[0])),
        n_0=float(solver_result.x[1]),
        R_s_Ohm_0=float(solver_result.x[2]),
        G_p_S_0=float(solver_result.x[3]),
        E_g_eV_0=float(solver_result.x[4]),
    )

    return types.ModelParametersFitResultLeastSquares(
