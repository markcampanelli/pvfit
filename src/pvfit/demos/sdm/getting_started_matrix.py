"""
PVfit: Getting-started demo for single-diode model (SDM) parameter inference from an
I-V measurement matrix.

Copyright 2023 Intelligent Measurement Systems LLC
"""

import importlib.resources
import os
from pprint import pprint
from typing import OrderedDict

from matplotlib import pyplot
import numpy
from numpy.polynomial import Polynomial
import pandas

from pvfit.common import E_hemispherical_tilted_W_per_m2_stc, T_degC_stc
from pvfit.measurement.iv.types import FTData, IVPerformanceMatrix, SpecSheetParameters
from pvfit.modeling.dc.common import Material
import pvfit.modeling.dc.single_diode.equation.simple.simulation as sde_sim
import pvfit.modeling.dc.single_diode.model.simple.auxiliary_equations as sdm_simple_ae
import pvfit.modeling.dc.single_diode.model.simple.inference_matrix as sdm_simple_inf_matrix
import pvfit.modeling.dc.single_diode.model.photoconductive_shunt.auxiliary_equations as sdm_ps_ae
import pvfit.modeling.dc.single_diode.model.photoconductive_shunt.inference_matrix as sdm_ps_inf_matrix

# By convention, variable names for numeric values include the units.

# Load PV module data.
# See https://pvpmc.sandia.gov/pv-research/pv-lifetime-project/pv-lifetime-modules/
datafile = "Sandia_PV_Module_P-Matrix-and-TempCo-Data_2019.xlsx"
path = importlib.resources.files("pvfit.demos.sdm.data") / datafile
sheets_matrix = pandas.read_excel(
    path, sheet_name=None, header=tuple(range(5)), skipfooter=16
)
sheets_coeffs = pandas.read_excel(path, sheet_name=None, header=tuple(range(35)))
N_s = {
    "19074-001": 96,
    "19074-002": 60,
    "19074-003": 60,
    "19074-004": 60,
    "19074-005": 60,
    "19074-006": 60,
    "19074-007": 60,
    "19074-008": 60,
    "19074-009": 72,
}
cell_tech = {
    "19074-001": "HIT Mono",
    "19074-002": "N-PERT Si",
    "19074-003": "poly-Si PERC",
    "19074-004": "poly-Si",
    "19074-005": "mono-Si",
    "19074-006": "poly-Si",
    "19074-007": "mono-Si PERC",
    "19074-008": "mono-Si PERC",
    "19074-009": "mono-Si PERC",
}

sheet_key = os.environ.get("PVFIT_SHEET_KEY", "19074-001")
sheet_key_found = False

for sheet_key_full, sheet_matrix_value in sheets_matrix.items():
    if sheet_key in sheet_key_full:
        sheet_key_found = True
        break

if not sheet_key_found:
    raise ValueError(f"{sheet_key} not found in {path}")

iv_performance_matrix = IVPerformanceMatrix(
    material=Material.xSi,  # Generic choice.
    N_s=N_s[sheet_key],
    I_sc_A=sheet_matrix_value.iloc[:, 3].to_numpy(),
    I_mp_A=sheet_matrix_value.iloc[:, 5].to_numpy(),
    V_mp_V=sheet_matrix_value.iloc[:, 6].to_numpy(),
    V_oc_V=sheet_matrix_value.iloc[:, 4].to_numpy(),
    E_W_per_m2=sheet_matrix_value.iloc[:, 2].to_numpy(),
    T_degC=sheet_matrix_value.iloc[:, 1].to_numpy(),
    E_W_per_m2_0=E_hemispherical_tilted_W_per_m2_stc,
    T_degC_0=T_degC_stc,
)

# Calculate coefficients that would be found on spec sheet.
sheet_coeffs_value = sheets_coeffs[sheet_key_full]
T_degC_0_coef_data = sheet_coeffs_value.iloc[1:4, 1]

I_sc_A_0_coef_data = sheet_coeffs_value.iloc[1:4, 3]
dI_sc_dT_A_per_degC_0 = float(
    Polynomial.fit(T_degC_0_coef_data, I_sc_A_0_coef_data, deg=1).convert().coef[1]
)

P_mp_W_0_coef_data = sheet_coeffs_value.iloc[1:4, 7]
dP_mp_dT_W_per_degC_0 = float(
    Polynomial.fit(T_degC_0_coef_data, P_mp_W_0_coef_data, deg=1).convert().coef[1]
)

V_oc_V_0_coef_data = sheet_coeffs_value.iloc[1:4, 4]
dV_oc_dT_V_per_degC_0 = float(
    Polynomial.fit(T_degC_0_coef_data, V_oc_V_0_coef_data, deg=1).convert().coef[1]
)

spec_sheet_parameters = SpecSheetParameters(
    material=iv_performance_matrix.material,
    N_s=iv_performance_matrix.N_s,
    I_sc_A_0=iv_performance_matrix.I_sc_A_0,
    I_mp_A_0=iv_performance_matrix.I_mp_A_0,
    V_mp_V_0=iv_performance_matrix.V_mp_V_0,
    V_oc_V_0=iv_performance_matrix.V_oc_V_0,
    dI_sc_dT_A_per_degC_0=dI_sc_dT_A_per_degC_0,
    dP_mp_dT_W_per_degC_0=dP_mp_dT_W_per_degC_0,
    dV_oc_dT_V_per_degC_0=dV_oc_dT_V_per_degC_0,
    E_W_per_m2_0=iv_performance_matrix.E_W_per_m2_0,
    T_degC_0=iv_performance_matrix.T_degC_0,
)

print(
    f"{sheet_key_full}: {cell_tech[sheet_key]} module with {iv_performance_matrix.N_s} "
    "cells in series."
)

print("\nI-V curve parameters at STC using specification-sheet data:")
pprint(
    OrderedDict(
        {
            "I_sc_A": spec_sheet_parameters.I_sc_A_0,
            "I_mp_A": spec_sheet_parameters.I_mp_A_0,
            "P_mp_W": spec_sheet_parameters.P_mp_W_0,
            "V_mp_V": spec_sheet_parameters.V_mp_V_0,
            "V_oc_V": spec_sheet_parameters.V_oc_V_0,
            "dI_sc_dT_A_per_degC_0": spec_sheet_parameters.dI_sc_dT_A_per_degC_0,
            "dP_mp_dT_W_per_degC_0": spec_sheet_parameters.dP_mp_dT_W_per_degC_0,
            "dV_oc_dT_V_per_degC_0": spec_sheet_parameters.dV_oc_dT_V_per_degC_0,
        }
    )
)

# Alternative operating condition.
F_alt = 0.5
T_degC_alt = 35.0

# F and/or T_degC can be vectorized, such as for a time-series of weather data.
F_series = numpy.array([0.95, 0.97, 0.99, 1.01, 0.97, 0.98])
T_degC_series = 35.0  # This scalar value will be approapriately broadcast.

# Fit simple single-diode model (SDM) to I-V performance matrix data.

# model_parameters_simple has both fittable and unfittable parameters. It can be used
# convienently in downstream functions, e.g., for power simulation.
print("\nFitting model parameters of simple SDM to performance matrix...")
# Additional fit outputs can be useful, but ignored here.
model_parameters_simple = sdm_simple_inf_matrix.fit(
    iv_performance_matrix=iv_performance_matrix,
).model_parameters
print("Fitting model parameters of simple SDM to performance matrix...done")

mape_mbpe_simple, _ = sdm_simple_inf_matrix.compute_fit_quality(
    iv_performance_matrix=iv_performance_matrix,
    model_parameters=model_parameters_simple,
)

print("\nModel parameters from simple-SDM fit to performance matrix:")
pprint(model_parameters_simple)
print("\nSimple SDM fit quality:")
pprint(mape_mbpe_simple)

# Compute parameters for I-V curve at STC using auxiliary equations to compute the
# model parameters passed to the single-diode equation (SDE).
iv_curve_parameters_0_simple = sde_sim.iv_curve_parameters(
    model_parameters=sdm_simple_ae.compute_sde_model_parameters(
        ft_data=FTData(F=1.0, T_degC=iv_performance_matrix.T_degC_0),
        model_parameters=model_parameters_simple,
    )
)

print("\nI-V curve parameters at STC using performance-matrix fit to simple SDM:")
pprint(iv_curve_parameters_0_simple)

# Save some fit I-V curve values for later.
I_sc_A_0_simple = iv_curve_parameters_0_simple.I_sc_A
I_mp_A_0_simple = iv_curve_parameters_0_simple.I_mp_A
V_mp_V_0_simple = iv_curve_parameters_0_simple.V_mp_V
V_oc_V_0_simple = iv_curve_parameters_0_simple.V_oc_V

# Compute at alternative operating condition.
iv_parameters_alt_simple = sde_sim.iv_curve_parameters(
    model_parameters=sdm_simple_ae.compute_sde_model_parameters(
        ft_data=FTData(F=F_alt, T_degC=T_degC_alt),
        model_parameters=model_parameters_simple,
    )
)

print(
    f"\nAlternative operating condition using simple SDM: F={F_alt}, T={T_degC_alt} °C"
)
pprint(iv_parameters_alt_simple)

# Computate a power time series.
P_mp_W_series = sde_sim.P_mp(
    model_parameters=sdm_simple_ae.compute_sde_model_parameters(
        ft_data=FTData(F=F_series, T_degC=T_degC_series),
        model_parameters=model_parameters_simple,
    )
)

print(
    "\nPower simulation over a series of effective-irradiance ratios at one "
    "temperature:"
)
print(f"F_series = {F_series}")
print(f"T_degC (fixed) = {T_degC_series}")
print(f"P_mp_W_series = {P_mp_W_series["P_mp_W"]}")

# Fit photoconductive-shunt single-diode model (SDM) to I-V performance matrix data.

# model_parameters_ps has both fittable and unfittable parameters. It can be used
# convienently in downstream functions, e.g., for power simulation.
print(
    "\nFitting model parameters of photoconductive-shunt SDM to performance matrix..."
)
# Additional fit outputs can be useful, but ignored here.
model_parameters_ps = sdm_ps_inf_matrix.fit(
    iv_performance_matrix=iv_performance_matrix,
).model_parameters
print(
    "Fitting model parameters of photoconductive-shunt SDM to performance matrix...done"
)

mape_mbpe_ps, _ = sdm_ps_inf_matrix.compute_fit_quality(
    iv_performance_matrix=iv_performance_matrix,
    model_parameters=model_parameters_ps,
)

print("\nModel parameters from photoconductive-shunt-SDM fit to performance matrix:")
pprint(model_parameters_ps)
print("Photoconductive-shunt SDM fit quality:")
pprint(mape_mbpe_ps)

# Compute parameters for I-V curve at STC using auxiliary equations to compute the
# model parameters passed to the single-diode equation (SDE).
iv_curve_parameters_0_ps = sde_sim.iv_curve_parameters(
    model_parameters=sdm_ps_ae.compute_sde_model_parameters(
        ft_data=FTData(F=1.0, T_degC=iv_performance_matrix.T_degC_0),
        model_parameters=model_parameters_ps,
    )
)

print(
    "\nI-V curve parameters at STC using performance-matrix fit to "
    "photoconductive-shunt SDM:"
)
pprint(iv_curve_parameters_0_ps)

# Save some fit I-V curve values for later.
I_sc_A_0_ps = iv_curve_parameters_0_ps.I_sc_A
I_mp_A_0_ps = iv_curve_parameters_0_ps.I_mp_A
V_mp_V_0_ps = iv_curve_parameters_0_ps.V_mp_V
V_oc_V_0_ps = iv_curve_parameters_0_ps.V_oc_V

# Compute at alternative operating condition.
iv_parameters_alt_ps = sde_sim.iv_curve_parameters(
    model_parameters=sdm_ps_ae.compute_sde_model_parameters(
        ft_data=FTData(F=F_alt, T_degC=T_degC_alt),
        model_parameters=model_parameters_ps,
    )
)

print(
    "\nAlternative operating condition using photoconductive-shunt SDM: "
    f"F={F_alt}, T={T_degC_alt} °C"
)
pprint(iv_parameters_alt_ps)

# Computate a power time series.
P_mp_W_series_ps = sde_sim.P_mp(
    model_parameters=sdm_simple_ae.compute_sde_model_parameters(
        ft_data=FTData(F=F_series, T_degC=T_degC_series),
        model_parameters=model_parameters_simple,
    )
)

print(
    "\nPower simulation over a series of effective-irradiance ratios at one "
    "temperature:"
)
print(f"F_series = {F_series}")
print(f"T_degC (fixed) = {T_degC_series}")
print(f"P_mp_W_series_ps = {P_mp_W_series_ps["P_mp_W"]}")

# Now make a nice comparison plot.
fig, axs = pyplot.subplots(1, 2, figsize=(12, 6))

# Plot simple SDM results.
ax = axs[0]
# Create color cycler.
cycler = pyplot.rcParams["axes.prop_cycle"]()

# Plot the data fits.
for idx, (F, T_degC) in enumerate(
    zip(iv_performance_matrix.F, iv_performance_matrix.T_degC)
):
    # Plot Isc, Pmp, and Voc with same colors as fit lines.
    color = next(cycler)["color"]
    ax.plot(
        iv_performance_matrix.ivft_data.V_V[3 * idx : 3 * idx + 3],
        iv_performance_matrix.ivft_data.I_A[3 * idx : 3 * idx + 3],
        "o",
        color=color,
    )
    V_V = numpy.linspace(0, iv_performance_matrix.ivft_data.V_V[3 * idx + 2], 101)
    ax.plot(
        V_V,
        sde_sim.I_at_V(
            V_V=V_V,
            model_parameters=sdm_simple_ae.compute_sde_model_parameters(
                ft_data=FTData(F=F, T_degC=T_degC),
                model_parameters=model_parameters_simple,
            ),
        )["I_A"],
    )

# Plot the LIC.
color = next(cycler)["color"]
ax.plot(
    [0.0, iv_parameters_alt_simple.V_mp_V, iv_parameters_alt_simple.V_oc_V],
    [iv_parameters_alt_simple.I_sc_A, iv_parameters_alt_simple.I_mp_A, 0.0],
    "*",
    color=color,
)
V_V = numpy.linspace(0, iv_parameters_alt_simple.V_oc_V, 101)
ax.plot(
    V_V,
    sde_sim.I_at_V(
        V_V=V_V,
        model_parameters=sdm_simple_ae.compute_sde_model_parameters(
            ft_data=FTData(F=F_alt, T_degC=T_degC_alt),
            model_parameters=model_parameters_simple,
        ),
    )["I_A"],
    "--",
    label=f"F={F_alt:.2f} suns, T={T_degC_alt:.0f} °C",
    color=color,
)
ax.set_title("Simple SDM")
ax.legend(loc="lower left")
ax.set_xlabel("V (V)")
ax.set_ylabel("I (A)")

# Plot photoconductive-shunt SDM results.
ax = axs[1]
# Create color cycler.
cycler = pyplot.rcParams["axes.prop_cycle"]()

# Plot the data fits.
for idx, (F, T_degC) in enumerate(
    zip(iv_performance_matrix.F, iv_performance_matrix.T_degC)
):
    # Plot Isc, Pmp, and Voc with same colors as fit lines.
    color = next(cycler)["color"]
    ax.plot(
        iv_performance_matrix.ivft_data.V_V[3 * idx : 3 * idx + 3],
        iv_performance_matrix.ivft_data.I_A[3 * idx : 3 * idx + 3],
        "o",
        color=color,
    )
    V_V = numpy.linspace(0, iv_performance_matrix.ivft_data.V_V[3 * idx + 2], 101)
    ax.plot(
        V_V,
        sde_sim.I_at_V(
            V_V=V_V,
            model_parameters=sdm_ps_ae.compute_sde_model_parameters(
                ft_data=FTData(F=F, T_degC=T_degC),
                model_parameters=model_parameters_ps,
            ),
        )["I_A"],
    )

# Plot the LIC.
color = next(cycler)["color"]
ax.plot(
    [0.0, iv_parameters_alt_ps.V_mp_V, iv_parameters_alt_ps.V_oc_V],
    [iv_parameters_alt_ps.I_sc_A, iv_parameters_alt_ps.I_mp_A, 0.0],
    "*",
    color=color,
)
V_V = numpy.linspace(0, iv_parameters_alt_ps.V_oc_V, 101)
ax.plot(
    V_V,
    sde_sim.I_at_V(
        V_V=V_V,
        model_parameters=sdm_ps_ae.compute_sde_model_parameters(
            ft_data=FTData(F=F_alt, T_degC=T_degC_alt),
            model_parameters=model_parameters_ps,
        ),
    )["I_A"],
    "--",
    label=f"F={F_alt:.2f} suns, T={T_degC_alt:.0f} °C",
    color=color,
)
ax.set_title("Photoconductive-Shunt SDM")
ax.legend(loc="lower left")
ax.set_xlabel("V (V)")
ax.set_ylabel("I (A)")

fig.suptitle(
    f"{sheet_key_full} {cell_tech[sheet_key]}: 6-Parameter Single-Diode Model (SDM) "
    "Fits to Matrix I-V Data",
    fontdict={"fontsize": 14},
)
fig.tight_layout()

pyplot.show()
