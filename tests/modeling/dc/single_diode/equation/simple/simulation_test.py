...
def test_dV_dI_at_I_explicit():
    ...
    n_1_mod_V = model_parameters.n * get_scaled_thermal_voltage(N_s=N_s, T_degC=T_degC)
...
def test_iv_parameters(iv_parameters_fixture):
    ...
    for field in fields(IVCurveParametersArray):
        numpy.testing.assert_allclose(
            getattr(iv_curve_parameters_got, field.name),
            getattr(iv_curve_parameters_expected, field.name),
        )
