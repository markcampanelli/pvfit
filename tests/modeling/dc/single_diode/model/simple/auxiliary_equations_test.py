...
def test_compute(params):
    ...
    model_parameters_got = sdm_simple_ae.compute_sde_model_parameters(
        ft_data=given["ft_data"],
        model_parameters=given["model_parameters"],
    )
    model_parameters_expected = expected["model_parameters"]

    for field in fields(sde_types.ModelParameters):
        numpy.testing.assert_allclose(
            getattr(model_parameters_got, field.name),
            getattr(model_parameters_expected, field.name),
            rtol=1e-05,
            atol=1e-08,
            err_msg=f"{field.name} got did not match expected",
        )
