...
def test_DataFunction():
    ...
    with pytest.raises(ValueError) as excinfo:
        pvfit.measurement.spectral_correction.api.DataFunction(x=x, y=y)
    ...
    # 1D case
    y = numpy.array([0, 0.5, 1.])
    f = pvfit.measurement.spectral_correction.api.DataFunction(x=x, y=y)
    f_duplicate = pvfit.measurement.spectral_correction.api.DataFunction(x=x, y=y)
...
def test_DataFunctionPositiveXNonnegativeY():
    ...
    y = numpy.array([0, 0.5, 1.])
    with pytest.raises(ValueError) as excinfo:
        pvfit.measurement.spectral_correction.api.DataFunctionPositiveXNonnegativeY(x=x, y=y)
...
def test_QuantumEfficiency():
    ...
    QE_fraction = pvfit.measurement.spectral_correction.api.QuantumEfficiency(lambda_nm=lambda_nm, QE=QE)
    ...
    QE_percent = \
        pvfit.measurement.spectral_correction.api.QuantumEfficiency(lambda_nm=lambda_nm, QE=numpy.array([0, 50, 100]), is_percent=True)
    ...
    QE_fraction = pvfit.measurement.spectral_correction.api.QuantumEfficiency(lambda_nm=lambda_nm, QE=numpy.array([0, 0.5, 1.]))
    QE_percent = \
        pvfit.measurement.spectral_correction.api.QuantumEfficiency(lambda_nm=lambda_nm, QE=numpy.array([0, 50, 100]), is_percent=True)
...
def test_SpectralIrradiance():
    ...
    E = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=E_W_per_m2_nm)
...
def test_SpectralResponivity():
    ...
    S = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=S_A_per_W)
...
def test_inner_product():
    ...
    f1 = pvfit.measurement.spectral_correction.api.DataFunction(x=x1, y=numpy.ones_like(x1))
    x2 = numpy.array([50, 150, 250])
    f2 = pvfit.measurement.spectral_correction.api.DataFunction(x=x2, y=numpy.full_like(x2, 2))
    inner_product_expected = 2. * (250 - 100)
    inner_product = pvfit.measurement.spectral_correction.api.inner_product(f1=f1, f2=f2)
    ...
    # Commutativity.
    numpy.testing.assert_equal(inner_product, pvfit.measurement.spectral_correction.api.inner_product(f1=f2, f2=f1))

    # Scalar-like computation, two non-constant lines.
    x1 = numpy.array([100, 200, 300])
    f1 = pvfit.measurement.spectral_correction.api.DataFunction(x=x1, y=numpy.array([0, 1, 2]))
    x2 = numpy.array([50, 150, 250, 350])
    f2 = pvfit.measurement.spectral_correction.api.DataFunction(x=x2, y=numpy.array([3, 2, 1, 0]))
    ...
    f1 = pvfit.measurement.spectral_correction.api.DataFunction(x=x1, y=numpy.array([1, 2]))
    x2 = numpy.array([50, 150])
    f2 = pvfit.measurement.spectral_correction.api.DataFunction(x=x2, y=numpy.array([3, 2]))
    ...
    f1 = pvfit.measurement.spectral_correction.api.DataFunction(x=x1, y=numpy.array([1, 2]))
    x2 = numpy.array([50, 150])
    f2 = pvfit.measurement.spectral_correction.api.DataFunction(x=x2, y=numpy.array([[3, 2], [5, 7]]))
    ...
    f = pvfit.measurement.spectral_correction.api.DataFunction(
        x=numpy.array([0, 1]), y=numpy.array([sys.float_info.max, sys.float_info.max]))
    with pytest.warns(Warning) as record:
        inner_product = pvfit.measurement.spectral_correction.api.inner_product(f1=f, f2=f)
    assert len(record) == 2
...
def test_M():
    ...
    shape = ()
    S_TD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 2))
    E_TD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    S_RD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 0.5))
    E_RD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    E_RC = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    M_expected = numpy.ones(shape)
    M = pvfit.measurement.spectral_correction.api.M(
        S_TD_OC=S_TD, E_TD_OC=E_TD, S_TD_RC=S_TD, E_TD_RC=E_RC, S_RD_OC=S_RD, E_RD_OC=E_RD, S_RD_RC=S_RD, E_RD_RC=E_RC)
    ...
    shape = (2,)
    S_TD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 2))
    E_TD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.ones((2, len(lambda_nm))))
    S_RD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 0.5))
    E_RD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    E_RC = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    M_expected = numpy.ones(shape)
    ...
    shape = (2, 2)
    S_TD = \
        pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=2*numpy.ones((2, 2, len(lambda_nm))))
    E_TD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    S_RD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 0.5))
    E_RD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    E_RC = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    ...
    # Scaling invariance.
    shape = ()
    S_TD_1 = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 2))
    E_TD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    S_RD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 0.5))
    E_RD = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    E_RC = pvfit.measurement.spectral_correction.api.SpectralIrradiance(lambda_nm=lambda_nm, E_W_per_m2_nm=numpy.full_like(lambda_nm, 1))
    # Scale S_TD_1 by 1/2.
    S_TD_2 = pvfit.measurement.spectral_correction.api.SpectralResponsivity(lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, 1))
    M_expected = numpy.ones(shape)
    M_1 = pvfit.measurement.spectral_correction.api.M(S_TD_OC=S_TD_1, E_TD_OC=E_TD, S_TD_RC=S_TD_1, E_TD_RC=E_RC, S_RD_OC=S_RD,
                                   E_RD_OC=E_RD, S_RD_RC=S_RD, E_RD_RC=E_RC)
    M_2 = pvfit.measurement.spectral_correction.api.M(S_TD_OC=S_TD_2, E_TD_OC=E_TD, S_TD_RC=S_TD_2, E_TD_RC=E_RC, S_RD_OC=S_RD,
                                   E_RD_OC=E_RD, S_RD_RC=S_RD, E_RD_RC=E_RC)
    numpy.testing.assert_equal(M_1, M_2)

    # Infinite Integrals.
    # Infinite M.
    S_TD = pvfit.measurement.spectral_correction.api.SpectralResponsivity(
        lambda_nm=lambda_nm, S_A_per_W=numpy.full_like(lambda_nm, sys.float_info.max))
    with pytest.warns(Warning) as record:
        M = pvfit.measurement.spectral_correction.api.M(S_TD_OC=S_TD, E_TD_OC=E_TD, S_TD_RC=S_TD_1, E_TD_RC=E_RC, S_RD_OC=S_RD,
                                     E_RD_OC=E_RD, S_RD_RC=S_RD, E_RD_RC=E_RC)
    assert len(record) == 3
    assert record[0].message.args[0] == "overflow encountered in multiply"
    assert record[1].message.args[0] == "Non-finite inner product detected."
    ...
    # NaN M.
    with pytest.warns(Warning) as record:
        M = pvfit.measurement.spectral_correction.api.M(S_TD_OC=S_TD, E_TD_OC=E_TD, S_TD_RC=S_TD, E_TD_RC=E_RC, S_RD_OC=S_RD, E_RD_OC=E_RD,
