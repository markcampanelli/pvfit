...
underlying numpy.ndarray data represting the various curves appearing in the
four integrals in the formula for M. See, for example, equation (5) in
https://doi.org/10.1002/ese3.190.

pvfit.measurement.spectral_correction.data has already created several useful
example data objects. For example, assuming one has loaded wavelength and
spectral responsivity data as 1D numpy arrays for the NIST test device (a x-Si
PV cell)...
"""
# lambda_nm = numpy.array([...])
lambda_nm = pvfit.measurement.spectral_correction.data.lambda_nm_TD_NIST
# S_A_per_W = numpy.array([...])
S_A_per_W = pvfit.measurement.spectral_correction.data.S_A_per_W_TD_NIST

"""
...one then creates a SpectralResponsivity object—
"""
S = pvfit.measurement.spectral_correction.api.SpectralResponsivity(
...
E_RD_RC = pvfit.measurement.spectral_correction.data.E_G173_global_tilt
