...
def current_sum_at_diode_node(
    *, V_V: Union[float, numpy.float64, numpy.ndarray], I_A: Union[float, numpy.float64, numpy.ndarray],
    ...
    n_1: Union[float, numpy.float64, numpy.ndarray], I_rs_h_A: Union[float, numpy.float64, numpy.ndarray],
    n_h: Union[float, numpy.float64, numpy.ndarray], R_h_Ohm: Union[float, numpy.float64, numpy.ndarray],
        R_s_Ohm: Union[float, numpy.float64, numpy.ndarray], G_p_S: Union[float, numpy.float64, numpy.ndarray]) -> dict:
    """
    ...
    result = pvfit.modeling.single_diode.equation.current_sum_at_diode_node(
        V_V=V_V, I_A=I_A, N_s=N_s, T_degC=T_degC, I_ph_A=I_ph_A, I_rs_1_A=I_rs_1_A, n_1=n_1, R_s_Ohm=R_s_Ohm,
        G_p_S=G_p_S)
    ...
    return ensure_numpy_scalars(
        dictionary={'I_sum_A': I_sum_A, 'T_K': result['T_K'], 'V_1_V': result['V_1_V'],
                    'n_1_mod_V': result['n_1_mod_V'], 'V_h_V': V_h_V, 'n_h_mod_V': n_h_mod_V})
...
def I_at_V(
    *, V_V: Union[float, numpy.float64, numpy.ndarray], N_s: Union[int, numpy.intc, numpy.ndarray],
    ...
    """
    # Optimizations.
    common_factor = (N_s * k_B_J_per_K * convert_temperature(T_degC, 'Celsius', 'Kelvin')) / q_C
    ...
    I_A_ic = I_ph_A - I_rs_1_A * numpy.expm1(V_1_V_ic / n_1_mod_V) - (I_rs_h_A * 0. * R_h_Ohm * n_h) - G_p_S * V_1_V_ic

    # Use closures in function definitions for Newton's method.
    def func(I_A):
        V_1_V = V_V + I_A * R_s_Ohm
        I_sum_A_partial = I_ph_A - I_rs_1_A * numpy.expm1(V_1_V / n_1_mod_V) - G_p_S * V_1_V - I_A
        return I_sum_A_partial - I_rs_h_A * numpy.expm1((V_1_V - R_h_Ohm * I_sum_A_partial) / n_h_mod_V)

    def fprime(I_A):
        V_1_V = V_V + I_A * R_s_Ohm
        common_term = \
            -I_rs_1_A * R_s_Ohm / n_1_mod_V * numpy.exp((V_V + I_A * R_s_Ohm) / n_1_mod_V) - G_p_S * R_s_Ohm - 1.
        return common_term - (I_rs_h_A / n_h_mod_V * (R_s_Ohm - R_h_Ohm * common_term)) * numpy.exp(
            (V_1_V - R_h_Ohm * (I_ph_A - I_rs_1_A * numpy.expm1(V_1_V / n_1_mod_V) - G_p_S * V_1_V - I_A)) / n_h_mod_V)
    ...
    result = current_sum_at_diode_node(
        V_V=V_V, I_A=I_A, N_s=N_s, T_degC=T_degC, I_ph_A=I_ph_A, I_rs_1_A=I_rs_1_A, n_1=n_1, I_rs_h_A=I_rs_h_A, n_h=n_h,
        R_h_Ohm=R_h_Ohm, R_s_Ohm=R_s_Ohm, G_p_S=G_p_S)
