...
import pvfit.modeling.dc.single_diode.model.simulation as sdm
...
filename, N_s, material = (
    os.path.join(os.path.dirname(__file__), "HIT05667.csv"),
    72,
    "x-Si",
)
data_table = pandas.read_csv(filename, skiprows=22, nrows=18, encoding="utf-8")
data_table.sort_values(
    ["Irradiance Corrected to (W/m2)", "Temperature Corrected to (degC)"], inplace=True
)
print(data_table)
...
for index, row in data_table.iterrows():
    V_V_data = numpy.concatenate((V_V_data, numpy.array([0.0, row[7], row[5]])))
    I_A_data = numpy.concatenate((I_A_data, numpy.array([row[4], row[6], 0.0])))
...
response = requests.post(
    "https://api.pvfit.app/v2/sdm/global",
...
{model_params_fit}\nfit_info = {response_json_dict['fit_info']}"
    ...
    print(
        f"API request failed with response status code \
{response.status_code}."
    )
...
if not success:
    model_params_fit = {
        ...
        "I_rs_1_A_0": 7.739216483430394e-11,
        "n_1_0": 1.086322575846391,
        "R_s_Ohm_0": 0.5948166451105333,
        "G_p_S_0": 0.00134157981991618,
        "E_g_eV_0": 1.1557066550514934,
    }
    print(
        f"API call failed: Falling back to previously computed fit parameters:\n\
model_params_fit = {model_params_fit}"
    )
...
P_mp_W_series = sdm.iv_params(F=F_series, T_degC=T_degC_series, **model_params_fit)[
    "P_mp_W"
]
...
for idx, (F, T_degC) in enumerate(zip(F_data_minimal, T_degC_data_minimal)):
    ...
    ax.plot(
        V_V,
        sdm.I_at_V_F_T(V_V=V_V, F=F, T_degC=T_degC, **model_params_fit)["I_A"],
...
ax.plot(
    V_V,
    sdm.I_at_V_F_T(V_V=V_V, F=F_alt, T_degC=T_degC_alt, **model_params_fit)["I_A"],
    "--",
