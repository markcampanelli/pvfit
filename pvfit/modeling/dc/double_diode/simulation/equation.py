"""PVfit: DC modeling for double-diode equation simulation."""
...
def current_sum_at_diode_node(
    *, V_V, I_A, N_s, T_degC, I_ph_A, I_rs_1_A, n_1, I_rs_2_A, n_2, R_s_Ohm, G_p_S
):
    """
    ...
    I_sum_A = (
        I_ph_A
        - I_rs_1_A * numpy.expm1(V_diode_V / n_mod_1_V)
        - I_rs_2_A * numpy.expm1(V_diode_V / n_mod_2_V)
        - G_p_S * V_diode_V
        - I_A
    )
    ...
    # numpy.float64).
    if isinstance(I_sum_A, numpy.float64) and any(
        map(
...
def I_at_V(
    *,
    ...
    V_diode_V_ic = V_V + 0.0 * R_s_Ohm

    # Compute initially with zero R_s_Ohm.
    I_A_ic = (
        I_ph_A
        - I_rs_1_A * numpy.expm1(V_diode_V_ic / n_mod_1_V)
        - I_rs_2_A * numpy.expm1(V_diode_V_ic / n_mod_2_V)
        - G_p_S * V_diode_V_ic
    )
    ...
    def fprime(I_A):
        V_diode_V = V_V + I_A * R_s_Ohm
        return (
            -I_rs_1_A * R_s_Ohm / n_mod_1_V * numpy.exp(V_diode_V / n_mod_1_V)
            - I_rs_2_A * R_s_Ohm / n_mod_2_V * numpy.exp(V_diode_V / n_mod_2_V)
            - G_p_S * R_s_Ohm
            - 1
        )
    ...
    # numpy.float64).
    if not isinstance(I_A, numpy.ndarray) and any(
        map(
            lambda x: isinstance(x, numpy.ndarray),
...
def I_at_V_d1(
    *,
    ...
    expr1 = (
        I_rs_1_A
        / result["n_mod_1_V"]
        * numpy.exp(result["V_diode_V"] / result["n_mod_1_V"])
        + I_rs_2_A
        / result["n_mod_2_V"]
        * numpy.exp(result["V_diode_V"] / result["n_mod_2_V"])
        + G_p_S
    )
    ...
    # Make sure to return an numpy.ndarray if any input was that type (undoes casting of rank-0 numpy.ndarray to
    # numpy.float64).
    if not isinstance(I_d1_V_S, numpy.ndarray) and isinstance(
        result["I_A"], numpy.ndarray
    ):
...
def V_oc(
    *,
    ...
    pos_idx = (
        0 < result["I_sum_A"]
    )  # Will cast a rank-0 array to a numpy scalar, due to numpy vaguarity!
    ...
    def func(V_V, I_ph_A, I_rs_1_A, n_mod_1_V, I_rs_2_A, n_mod_2_V, R_s_Ohm, G_p_S):
        ...
        return (
            I_ph_A
            - I_rs_1_A * numpy.expm1(V_V / n_mod_1_V)
            - I_rs_2_A * numpy.expm1(V_V / n_mod_2_V)
            - G_p_S * V_V
        )
    ...
    def fprime(V_V, I_ph_A, I_rs_1_A, n_mod_1_V, I_rs_2_A, n_mod_2_V, R_s_Ohm, G_p_S):
        ...
        return (
            -I_rs_1_A / n_mod_1_V * numpy.exp(V_V / n_mod_1_V)
            - I_rs_2_A / n_mod_2_V * numpy.exp(V_V / n_mod_2_V)
            - G_p_S
        )

    def qudratic_estimate(
        *, V_V, I_ph_A, I_rs_1_A, n_mod_1_V, I_rs_2_A, n_mod_2_V, R_s_Ohm, G_p_S
    ):
        ...
        a = (
            -(
                I_rs_1_A / n_mod_1_V**2 * numpy.exp(V_V / n_mod_1_V)
                + I_rs_2_A / n_mod_2_V**2.0 * numpy.exp(V_V / n_mod_2_V)
            )
            / 2
        )
        b = fprime(
        ...
        return V_V + (-b - numpy.sqrt(b**2.0 - 4.0 * a * c)) / (2.0 * a)
    ...
    while numpy.any(pos_idx):
        ...
        V_oc_V_est[pos_idx] = qudratic_estimate(
            V_V=V_oc_V_est[pos_idx],
            I_ph_A=I_ph_A[pos_idx],
            I_rs_1_A=I_rs_1_A[pos_idx],
            n_mod_1_V=n_mod_1_V[pos_idx],
            I_rs_2_A=I_rs_2_A[pos_idx],
            n_mod_2_V=n_mod_2_V[pos_idx],
            R_s_Ohm=R_s_Ohm[pos_idx],
            G_p_S=G_p_S[pos_idx],
        )
        ...
        while numpy.any(large_neg_idx):
            ...
            V_oc_V_est[large_neg_idx] = (
                V_oc_V_est[large_neg_idx] + V_oc_V_est_last[large_neg_idx]
            ) / 2.0
            large_neg_idx = (
                func(
                    V_V=V_oc_V_est,
    ...
    V_oc_V = newton(
        ...
        V_oc_V_est,
        fprime=fprime,
        args=(I_ph_A, I_rs_1_A, n_mod_1_V, I_rs_2_A, n_mod_2_V, R_s_Ohm, G_p_S),
        **newton_options,
    )
...
def V_at_I(
    *,
    ...
    V_V_ic = numpy.where(
        I_A <= I_knot_A, V_oc_V - I_A * R_oc_Ohm, R_sc_Ohm * (I_sc_A - I_A)
    )
    ...
    def fprime(V_V):
        V_diode_V = V_V + I_A * R_s_Ohm
        return (
            -I_rs_1_A / n_mod_1_V * numpy.exp(V_diode_V / n_mod_1_V)
            - I_rs_2_A / n_mod_2_V * numpy.exp(V_diode_V / n_mod_2_V)
            - G_p_S
        )
    ...
    # Make sure to return numpy.ndarray if any input was that type (undoes casting of rank-0 numpy.ndarray to
    # numpy.float64).
    if not isinstance(V_V, numpy.ndarray) and any(
        map(
            lambda x: isinstance(x, numpy.ndarray),
...
def P_mp(
    *,
    ...
    R_s_Ohm,
    G_p_S,
    minimize_scalar_bounded_options=None,
    newton_options=None,
):
    """
    ...
    def opposite_P_at_V(
        V_V, N_s, T_degC, I_ph_A, I_rs_1_A, n_1, I_rs_2_A, n_2, R_s_Ohm, G_p_S
    ):
        return -P_at_V(
            V_V=V_V,
            N_s=N_s,
    ...
    array_min_func = numpy.frompyfunc(
        lambda V_oc_V, N_s, T_degC, I_ph_A, I_rs_1_A, n_1, I_rs_2_A, n_2, R_s_Ohm, G_p_S: minimize_scalar(
            opposite_P_at_V,
            bounds=(0.0, V_oc_V),
            args=(N_s, T_degC, I_ph_A, I_rs_1_A, n_1, I_rs_2_A, n_2, R_s_Ohm, G_p_S),
            method="bounded",
    ...
    res_array = array_min_func(
        V_oc_V, N_s, T_degC, I_ph_A, I_rs_1_A, n_1, I_rs_2_A, n_2, R_s_Ohm, G_p_S
    )
    ...
    with numpy.errstate(divide="ignore", invalid="ignore"):
        I_mp_A = numpy.where(V_mp_V != 0, P_mp_W / V_mp_V, 0.0)
...
def FF(
    *,
    ...
    with numpy.errstate(divide="ignore", invalid="ignore"):
        FF = numpy.where(denominator != 0, result["P_mp_W"] / denominator, 0.0)
