    ...
    params=[
        ...
        {  # Can handle all rank-1 singleton array inputs.
            ...
            'I_sum_A_expected': numpy.array([7. - 6.e-7 * numpy.expm1(
                q_C * (0.5 + 3. * 0.1) / (1 * 1.25 * k_B_J_per_K * T_K_stc)) - 0.005 * (0.5 + 3. * 0.1) - 3.]),
            'T_K_expected': numpy.float64(T_K_stc),
            'V_1_V_expected': numpy.array([0.5 + 3. * 0.1]),
            'n_1_mod_V_expected': numpy.array([(1 * 1.25 * k_B_J_per_K * T_K_stc) / q_C])
        },
        {  # Can handle all rank-1 non-singleton array inputs.
            'V_V': numpy.array([0.5, 0., 0.]),
            'I_A': numpy.array([0., 0., 3.]),
            'I_ph_A': numpy.array([7., 7., 7.]),
            'I_rs_1_A': numpy.array([6.e-7, 6.e-7, 6.e-7]),
            ...
            'T_K_expected': numpy.array([273.15 + T_degC_stc / 2, T_K_stc, 273.15 + 2 * T_degC_stc]),
            'V_1_V_expected': numpy.array([0.5, 0., 3. * 0.1]),
            'n_1_mod_V_expected': (numpy.array([1, 60, 96]) * 1.25 * k_B_J_per_K * convert_temperature(
                numpy.array([T_degC_stc / 2, T_degC_stc, 2 * T_degC_stc]), 'Celsius', 'Kelvin')) / q_C
        },
        {  # Can handle mixed inputs with python floats.
            ...
            'I_sum_A_expected': numpy.array([
                ...
                7.,
                7. - 6.e-7 * numpy.expm1(q_C * 3. * 0.1 / (1 * 1.25 * k_B_J_per_K * T_K_stc)) - 0.005 * 3. * 0.1 - 3.
            ]),
            'T_K_expected': numpy.float64(T_K_stc),
            'V_1_V_expected': numpy.array([0.5, 0., 3. * 0.1]),
            'n_1_mod_V_expected': numpy.float64((1 * 1.25 * k_B_J_per_K * T_K_stc) / q_C)
        ...
        {  # Can handle mixed inputs and zero shunt conductance with positive series resistance.
            ...
            'I_sum_A_expected': numpy.float64(
                7. - 6.e-7 * numpy.expm1(q_C * (0.5 + 3. * 0.1) / (1 * 1.25 * k_B_J_per_K * T_K_stc)) - 3.),
            'T_K_expected': numpy.float64(T_K_stc),
            'V_1_V_expected': numpy.float64(0.5 + 3 * 0.1),
            'n_1_mod_V_expected': numpy.float64((1 * 1.25 * k_B_J_per_K * T_K_stc) / q_C)
        },
        {  # Can handle mixed inputs and zero shunt conductance with zero series resistance
            ...
            'T_degC': T_degC_stc,
            'I_sum_A_expected': numpy.array(
                [7. - 6.e-7 * numpy.expm1(q_C * 0.5 / (1 * 1.25 * k_B_J_per_K * T_K_stc)) - 3.]),
            'T_K_expected': numpy.float64(T_K_stc),
...
def test_I_at_V_explicit():
    ...
    N_s = 1
    T_degC = T_degC_stc
    I_A_expected = I_ph_A - I_rs_1_A * numpy.expm1(q_C * V_V / (N_s * n_1 * k_B_J_per_K * T_K_stc)) - G_p_S * V_V

    I_A = equation.I_at_V(
        V_V=V_V, N_s=N_s, T_degC=T_degC, I_ph_A=I_ph_A, I_rs_1_A=I_rs_1_A, n_1=n_1, R_s_Ohm=R_s_Ohm, G_p_S=G_p_S)['I_A']
...
def test_P_at_V_explicit():
    ...
    P_W_expected = numpy.float64(
        V_V * (I_ph_A - I_rs_1_A * numpy.expm1(q_C * V_V / (N_s * n_1 * k_B_J_per_K * T_K_stc)) - G_p_S * V_V))

    P_W = equation.P_at_V(
        V_V=V_V, N_s=N_s, T_degC=T_degC, I_ph_A=I_ph_A, I_rs_1_A=I_rs_1_A, n_1=n_1, R_s_Ohm=R_s_Ohm, G_p_S=G_p_S)['P_W']
