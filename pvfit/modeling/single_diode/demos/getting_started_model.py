...
from pvfit.common.constants import T_stc_degC
...
I_sc_0_A_meas_row_idx = numpy.logical_and(
    data_table.values[:, 2] == 25, data_table.values[:, 3] == 1000)
I_sc_0_A_meas = numpy.float(data_table.values[I_sc_0_A_meas_row_idx, 4])
...
for index, row in data_table.iterrows():
    ...
    F_data = numpy.concatenate((F_data, numpy.full(3, row[4] / I_sc_0_A_meas)))
    T_degC_data = numpy.concatenate((T_degC_data, numpy.full(3, row[2])))
    F_data_minimal = numpy.concatenate(
        (F_data_minimal, numpy.array([row[4] / I_sc_0_A_meas])))
...
print(f"I_sc_0_A_meas = {I_sc_0_A_meas}")
print(f"I_A_data =\n{I_A_data}")
print(f"V_V_data =\n{V_V_data}")
print(f"F_data =\n{F_data}")
print(F"T_degC_data =\n{T_degC_data}")
...
{response.status_code}.")
    ...
    if response_json_dict['success']:
        ...
        # Switch the units position in variable names.
        model_params_fit = {'N_s': model_params_fit['N_s'],
                            'T_0_degC': model_params_fit['T_degC_0'],
                            'I_sc_0_A': model_params_fit['I_sc_A_0'],
                            'I_rs_0_A': model_params_fit['I_rs_A_0'],
                            'n_0': model_params_fit['n_0'],
                            'R_s_0_Ohm': model_params_fit['R_s_Ohm_0'],
                            'G_p_0_S': model_params_fit['G_p_S_0'],
                            'E_g_0_eV': model_params_fit['E_g_eV_0']}
...
if not success:
    model_params_fit = {
        'N_s': 72, 'T_0_degC': 25.0, 'I_sc_0_A': 5.531975277850156,
        'I_rs_0_A': 7.739216483430394e-11, 'n_0': 1.086322575846391,
        'R_s_0_Ohm': 0.5948166451105333, 'G_p_0_S': 0.00134157981991618,
        'E_g_0_eV': 1.1557066550514934}
...
result = sdm.derived_params(F=1, T_degC=T_stc_degC, **model_params_fit)
